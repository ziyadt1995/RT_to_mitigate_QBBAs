...
def load_imagenet(n_ex = 1000, size=224):
    IMAGENET_SL = size
    IMAGENET_PATH = '../test_images/Sample_1000'
    ...
    #imagenet_loader = DataLoader(imagenet, batch_size=100, shuffle=False, num_workers=8, pin_memory=True)

    imagenet_loader = torch.utils.data.DataLoader(
            dset.ImageFolder('../test_images/Sample_1000', transform=val_transform),
            batch_size=1000, shuffle=False,
            num_workers=8, pin_memory=True)
    print("yes")
    #x_test, y_test = next(iter(imagenet_loader))
    for x, y in tqdm(imagenet_loader):
        x_test=x+0
        y_test=y+0
    print(x_test.shape)
...
class Dataset(object):
    def __init__(self, name, config):
        ...
        if self.name == 'cifar10':
            ...
            data_path = data_path_join('test_images\Sample_1000')
            self.data = ImagenetValidData(data_dir='../test_images/Sample_1000')
