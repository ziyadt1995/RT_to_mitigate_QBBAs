...
class ResNet(nn.Module):
    ...
    def _forward(self, x, with_latent=False, fake_relu=False, no_relu=False, RR=True, RRC=False, rfd=False):
        if(RR==True):
            x=transforms.RandomRotation(degrees=(-10,10))(x)
            #random transformation from cihang xies paper
            #a=random.randint(208, 224)
            #x=transforms.Resize(size=(a,a))(x)
            #x=transforms.CenterCrop(224)(x)
        if(RRC==True):
            x=transforms.RandomResizedCrop(size=(224,224),scale=(0.8,1))(x)
